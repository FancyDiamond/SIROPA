<!doctype html>
<html lang="{{ app()->getLocale() }}">
  <head>
    <!-- Required meta tags -->
    <meta charset="utf-8">
    <meta name="viewport" content="width=device-width, initial-scale=1, shrink-to-fit=no">
    <title>Usuarios</title>

    <!-- Bootstrap CSS -->
    <!--
    <link rel="stylesheet" href="css/bootstrap.min.css">
    <link rel="stylesheet" href="Font-Awesome-master/web-fonts-with-css/css/fontawesome-all.min.css">
    <link rel="stylesheet" type="text/css" href="font-awesome/css/font-awesome.min.css">
    <link rel="stylesheet" type="text/css" href="css/global.css">
    -->

    <link rel="stylesheet" type="text/css" href="{{asset('My_Desing/css/bootstrap.min.css')}}">
    <link rel="stylesheet" type="text/css" href="{{asset('My_Desing/Font-Awesome-master/web-fonts-with-css/css/fontawesome-all.min.css')}}">
    <link rel="stylesheet" type="text/css" href="{{asset('My_Desing/font-awesome/css/font-awesome.min.css')}}">
    <link rel="stylesheet" type="text/css" href="{{asset('My_Desing/css/global.css')}}">

  </head>
  <body background="{{ asset('My_Desing/images/wallpaper2you_27497.jpg') }}">

    <div class="container-fluid">
      <div class="row">
        <div class="col-md-4 col-sm-4 col-xs-12" ></div>
        <div class="col-md-4 col-sm-4 col-xs-12">
          <!--form start-->
          <!--<form class="form-container" method="POST" action="ruta" >-->
          {!! Form:: open(['route'=>'usuario.store' ,'method'=>'POST', 'class'=>'form-container']) !!}
            <!--<h1>Registro de Usuarios</h1>-->
            <legend>Registro de Usuarios</legend>
            <hr>
            <div class="form-group">
              <label for="Nombre" class="fa fa-user">Nombre</label>
              <input type="text" class="form-control" id="Nombre" placeholder="Nombre" name="Nombre" required="" autofocus>
            </div>
            <div class="form-group">
              <label for="Clavefuncionario" class="fa fa-key">Clave de Funcionario</label>
              <input type="text" class="form-control" id="Clavefuncionario" placeholder="Clave de funcionario" name="Clavefuncionario" required="">
            </div>
            <div class="form-group">
              <label for="Email" class="fas fa-evelop">Correo</label>
              <input type="email" class="form-control" id="Email" placeholder="Correo" name="Email" required="">
            </div>
            <div class="form-group">
              <label for="Password" class="fa fa-key">Contraseña</label>
              <input type="password" class="form-control" id="Password" placeholder="Contraseña" name="Password" required="">
            </div>
            <div class="form-group">
              <label for="Telefono" class="fa-headphones">Teléfono</label>
              <input type="number" class="form-control" id="Telefono" placeholder="Teléfono" name="Telefono" required="">
            </div>
            <div class="form-group">
              <label for="Direccion" class="far fa-address-card">Dirección</label>
              <input type="text" class="form-control" id="Direccion" placeholder="Dirección" name="Direccion" required="">
            </div>
            <div class="form-group">
              <label for="Municipio" name="Municipio">Municipio</label>
              <select name="Municipio" id="Municipio" class="form-control">
                <option value="Tlaltenango de Sanchez Roman">Tlaltenango de Sanchez Roman</option>
                <option value="Tepechitlan">Tepechitlan</option>
                <option value="Momax">Momax</option>
                <option value="Atolinga">Atolinga</option>
                <option value="Santa Maria de la Paz">Santa Maria de la Paz</option>
                <option value="Teúl de Gonzalez Ortega">Teúl de Gonzalez Ortega</option>
                <option value="Florencia de Benito Juárez">Florencia de Benito Juárez</option>
              </select>
            </div>
            <div class="form-group">
              <label for="Puesto" name="Puesto">Puesto</label>
              <select name="Puesto" id="Puesto" class="form-control">
                <option value="Controlador">Contralor</option>
                <option value="Obra Pública">Obra Pública</option>
                <option value="Otro">Función Pública</option>
              </select>
            </div>
            <div>
               <button type="submit" class="btn btn-success btn-block">Guardar</button>
            </div>
          {!! Form::close() !!}
          <!--</form>-->
          <!--form end-->
        </div>
        <div class="col-md-4 col-sm-4 col-xs-12"></div>
      </div>
    </div>

    <!-- jQuery first, then Tether, then Bootstrap JS. -->
    <script src="https://code.jquery.com/jquery-3.1.1.slim.min.js" integrity="sha384-A7FZj7v+d/sdmMqp/nOQwliLvUsJfDHW+k9Omg/a/EheAdgtzNs3hpfag6Ed950n" crossorigin="anonymous"></script>
    <script src="https://cdnjs.cloudflare.com/ajax/libs/tether/1.4.0/js/tether.min.js" integrity="sha384-DztdAPBWPRXSA/3eYEEUWrWCy7G5KFbe8fFjk5JAIxUYHKkDx6Qin1DkWx51bBrb" crossorigin="anonymous"></script>
    <script src="js/bootstrap.min.js"></script>
  </body>
</html>